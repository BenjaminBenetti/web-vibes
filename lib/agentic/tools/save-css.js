...
class SaveCSSTool extends AgenticTool {
  ...
  constructor(hackService) {
    ...
    super(
      "save_vibe_css",
      "Save CSS code to the current vibe/hack under edit. Can either replace existing vibe CSS code or append to it. You should call apply_hack after this to apply the changes.",
      schema
    );
  ...
  /**
   * Get user-friendly action message for vibe CSS saving
   * @returns {string} Action message
   */
  getActionMessage() {
    return "Updating vibe CSS styles";
  }

  /**
   * Get user-friendly completion message for vibe CSS saving
   * @returns {string} Completion message
   */
  getCompletionMessage() {
    return "Vibe CSS styles updated";
  }

  /**
   * Get user-friendly error message for vibe CSS saving failures
   * @param {string} error - The actual error
   * @returns {string} Error message
   */
  getErrorMessage(error) {
    return `Failed to update vibe CSS: ${error}`;
  }
}
