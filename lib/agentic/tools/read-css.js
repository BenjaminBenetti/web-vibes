...
class ReadCSSTool extends AgenticTool {
  ...
  constructor(hackService) {
    ...
    super(
      "read_css",
      "Read the currently applied CSS edits. This is the CSS you are working on. Returns the current CSS code and optionally metadata about it.",
      schema,
      false // This tool only reads data
    );
  ...
  /**
   * Get user-friendly action message for vibe CSS reading
   * @returns {string} Action message
   */
  getActionMessage() {
    return "Reading vibe CSS code";
  }

  /**
   * Get user-friendly completion message for vibe CSS reading
   * @returns {string} Completion message
   */
  getCompletionMessage() {
    return "Vibe CSS code analyzed";
  }

  /**
   * Get user-friendly error message for vibe CSS reading failures
   * @param {string} error - The actual error
   * @returns {string} Error message
   */
  getErrorMessage(error) {
    return `Failed to read vibe CSS: ${error}`;
  }
}
