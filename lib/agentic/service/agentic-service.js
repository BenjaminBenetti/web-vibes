...
class AgenticService {
  ...
  constructor(aiService, tools = []) {
     ...
     * Currently set to 25KB as requested.
     * @type {number}
     */
    this.MAX_MESSAGE_SIZE = 25 * 1024; // 25KB
     ...
     * Currently set to 100KB as requested.
     * @type {number}
     */
    this.MAX_HISTORY_SIZE = 100 * 1024; // 100KB
  ...
  async startAgenticLoop(userRequest, options = {}) {
    try {
      ...
      this.currentIteration = 0;
      const shouldResetHistory = options.resetHistory || this.conversationHistory.length === 0;
      ...
      // If this is a fresh conversation, bootstrap with system prompt and detailed initial instructions
      if (shouldResetHistory) {
...
Start by reading the existing website code, using tools that include the word "website" to understand the current state, then proceed with the requested modifications.`;
        ...
        // Ensure messages do not exceed the maximum size
        this._addMessageToHistory({
          role: "system",
          content: this._truncateContent(systemPrompt),
        });
  ...
  async runAgenticLoop(verbose = false) {
    while (this.currentIteration < this.maxIterations) {
      ...
      try {
        ...
        const { readTools, writeTools, mixedUsageMessage } = this.separateReadWriteTools(toolCalls);
...
6. If you complete the user's request successfully, provide a clear summary of what was accomplished.

7. If you encounter errors, try to fix them or suggest alternatives.

8. Always call apply_hack after saving changes so the user can see the results.
...
This ensures you can see the current state before making modifications.`;
  ...
  _truncateContent(content) {
    try {
      ...
      const truncationNotice = "\n\n[⚠️ Content truncated due to exceeding 25KB limit]";
      ...
      if (!encoder) {
        ...
        const allowedBytes = maxBytes - Buffer.byteLength(truncationNotice, "utf8");
  ...
  _calculateHistorySize() {
    try {
      ...
      if (!encoder) {
        ...
        return Buffer.byteLength(JSON.stringify(this.conversationHistory), "utf8");
      }
  ...
  _trimHistory() {
    ...
    if (trimmedCount > 0) {
      trimmingMessage = `🗂️ Conversation history trimmed: Removed ${trimmedCount} oldest messages to stay under 100KB limit. History reduced from ${initialCount} to ${finalCount} messages (${Math.round(initialSize / 1024)}KB → ${Math.round(finalSize / 1024)}KB).`;
    }
