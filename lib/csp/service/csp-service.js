...
class CSPService {
  ...
  /**
   * Enable service worker blocking for a hostname
   * @param {string} hostname - The hostname to enable service worker blocking for
   * @returns {Promise<CSPSettings>} Updated CSP settings
   */
  async enableServiceWorkerBlocking(hostname) {
    const settings = await this.getCSPSettings(hostname);

    if (!settings.isServiceWorkerBlockingEnabled()) {
      settings.enableServiceWorkerBlocking();
      await this.repository.saveCSPSettings(settings);
  ...
  /**
   * Disable service worker blocking for a hostname
   * @param {string} hostname - The hostname to disable service worker blocking for
   * @returns {Promise<CSPSettings>} Updated CSP settings
   */
  async disableServiceWorkerBlocking(hostname) {
    const settings = await this.getCSPSettings(hostname);

    if (settings.isServiceWorkerBlockingEnabled()) {
      settings.disableServiceWorkerBlocking();
      await this.repository.saveCSPSettings(settings);
  ...
  /**
   * Toggle service worker blocking for a hostname
   * @param {string} hostname - The hostname to toggle service worker blocking for
   * @returns {Promise<CSPSettings>} Updated CSP settings
   */
  async toggleServiceWorkerBlocking(hostname) {
    const settings = await this.getCSPSettings(hostname);

    if (settings.isServiceWorkerBlockingEnabled()) {
      return await this.disableServiceWorkerBlocking(hostname);
    } else {
      return await this.enableServiceWorkerBlocking(hostname);
    }
  }
  ...
  async isServiceWorkerBlockingEnabled(hostname) {
    const settings = await this.getCSPSettings(hostname);
    return settings.isServiceWorkerBlockingEnabled();
  }
  ...
  /**
   * Get all hostnames with service worker blocking enabled
   * @returns {Promise<string[]>} Array of hostnames with service worker blocking enabled
   */
  async getServiceWorkerBlockingEnabledHostnames() {
    const allSettings = await this.repository.getAllCSPSettings();
    return Object.keys(allSettings).filter((hostname) =>
      allSettings[hostname].isServiceWorkerBlockingEnabled()
    );
  }
  ...
  async initializeCSPRules() {
    try {
      ...
      let cspRulesCount = 0;
      let serviceWorkerRulesCount = 0;

      // Add rules based on individual settings
      for (const [hostname, settings] of Object.entries(allSettings)) {
        ...
        if (settings.isServiceWorkerBlockingEnabled()) {
          await this._addServiceWorkerBlockingRule(hostname);
          serviceWorkerRulesCount++;
        }
      }

      console.log(
        `Initialized ${cspRulesCount} CSP rules and ${serviceWorkerRulesCount} service worker blocking rules`
      );
    } catch (error) {
    ...
    try {
      ...
      const rule = {
        ...
        condition: {
          ...
          // Match service worker files on the specific hostname
          regexFilter: `^https?://([^/]*\\.)?${hostname.replace(
            /\./g,
            "\\."
