...
class HackService {
  ...
  /**
   * Import vibes to the current site from a .grove file
   * @param {string} hostname - The target hostname to import vibes to
   * @param {Array|Object} vibesData - The vibes data from .grove file
   * @returns {Promise<Hack[]>} Updated array of hacks
   */
  async importVibes(hostname, vibesData) {
    try {
      ...
      // Handle different data formats from .grove files
      if (Array.isArray(vibesData)) {
        // Direct array of hacks
        hacksArray = vibesData;
      } else if (vibesData.hacks && Array.isArray(vibesData.hacks)) {
        // Object with hacks array (from export format)
        hacksArray = vibesData.hacks;
      } else {
      ...
      const validHacks = [];
      for (const hackData of hacksArray) {
