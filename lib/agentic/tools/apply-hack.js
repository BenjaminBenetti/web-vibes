  ...
  async run(parameters) {
    try {
          ...
          const injectionResults = await chrome.scripting.executeScript({
            ...
            func: (codeString, hackId, hackName) => {
              try {
                ...
                console.log(`Web Vibes: Executed JavaScript for hack "${hackName}"`);
                return { success: true };
              } catch (err) {
                console.error(`Web Vibes: error executing hack "${hackName}":`, err);
      ...
      if (overallSuccess) {
        return this.formatSuccess(
          {
            ...
            hasCSS: !!(hackData.cssCode && hackData.cssCode.trim()),
            hasJS: !!(hackData.jsCode && hackData.jsCode.trim()),
        ...
        const errors = [];
        if (!cssSuccess) errors.push(`CSS: ${cssResult?.error || 'Unknown error'}`);
        if (!jsSuccess) errors.push(`JavaScript: ${jsResult?.error || 'Unknown error'}`);

        return this.formatError(
          `Failed to apply hack: ${errors.join(', ')}`,
          {
            cssResult,
            jsResult,
            tabUrl: activeTab.url
