...
7. Always call apply_hack after saving changes so the user can see the results.
...
- Always read the content of the website first to understand how you should modify it`;
  ...
  parseToolCalls(responseText) {
    try {
      ...
      // For each tool call, find its corresponding parameters
      for (let i = 0; i < toolMatches.length; i++) {
        const toolMatch = toolMatches[i];
        const nextToolMatch = toolMatches[i + 1];

        // Extract text from current tool call to next tool call (or end of text)
        const startIndex = toolMatch.index + toolMatch.fullMatch.length;
        const endIndex = nextToolMatch
          ? nextToolMatch.index
          : responseText.length;
        const sectionText = responseText.substring(startIndex, endIndex);

        let parameters = {};

        // Look for PARAMETERS: line in this section
        const parametersMatch = sectionText.match(
          /PARAMETERS:\s*([\s\S]*?)(?=\n\n|\nTOOL_CALL|\n[A-Z][a-z]|\n[^{]|$)/
        );
        ...
        if (parametersMatch) {
          ...
          console.log(
            `Raw parameters text for ${toolMatch.name}:`,
          ...
          if (jsonStart !== -1) {
            ...
            for (let j = jsonStart; j < parametersText.length; j++) {
              if (parametersText[j] === "{") braceCount++;
              if (parametersText[j] === "}") braceCount--;
              if (braceCount === 0) {
                jsonEnd = j;
                break;
            ...
            console.log(
              `Extracted JSON string for ${toolMatch.name}:`,
            ...
            try {
              ...
              console.log(
                `Parsed parameters for ${toolMatch.name}:`,
                parameters
              );
            } catch (jsonError) {
              console.warn(
                `Failed to parse tool parameters JSON for ${toolMatch.name}:`,
                jsonError
              );
            ...
            console.warn(
              `No opening brace found in parameters text for ${toolMatch.name}:`,
              JSON.stringify(parametersText)
            );
          }
        } else {
          console.warn(
            `No PARAMETERS section found for tool ${toolMatch.name}`
          );
        }
