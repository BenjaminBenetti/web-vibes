    ...
    super(
      ...
      "Search webpage content by keyword and return HTML surrounding matches with configurable context",
      {
        ...
        properties: {
          ...
          contextLines: {
            type: "number",
            description: "Number of lines of HTML context before and after each match (default: 50)",
          },
          caseSensitive: {
            type: "boolean",
    ...
    try {
      ...
      try {
        const response = await chrome.tabs.sendMessage(activeTab.id, {
          ...
          contextLines: params.contextLines || 50,
        ...
        if (response && response.success) {
          return this.formatSuccess(
            {
              message: `Found ${response.totalFound} matches for "${response.keyword}"`,
