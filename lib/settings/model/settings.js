...
class Settings {
  ...
  setAICredentials(aiProvider, credentials) {
    ...
    this.aiCredentials[aiProvider] = credentials;
    this.updatedAt = new Date();
  }
   ...
   * @param {string} aiProvider - The AI provider identifier
   * @returns {Object} The credentials object or empty object if not found
   */
  getAICredentials(aiProvider) {
    return this.aiCredentials[aiProvider] || {};
  }
  ...
  clearAICredentials(aiProvider) {
    delete this.aiCredentials[aiProvider];
    this.updatedAt = new Date();
  }
  ...
  toJSON() {
    return {
      ...
      aiCredentials: this.aiCredentials,
  ...
  static isValid(data) {
    return (
      ...
      (data.aiCredentials === undefined ||
        typeof data.aiCredentials === "object")
    );
  }
   ...
   * @returns {boolean} True if AI is properly configured
   */
  isAIConfigured() {
    ...
    if (!aiData.requiresApiKey) {
      return true;
    }

    const credentials = this.getAICredentials(this.selectedAI);
    const credentialFields = aiData.credentialFields || {};

    // Check if all required credential fields are present
    for (const [fieldName, fieldConfig] of Object.entries(credentialFields)) {
      if (
        fieldConfig.required &&
        (!credentials[fieldName] || credentials[fieldName].trim() === "")
      ) {
        return false;
      }
    }
