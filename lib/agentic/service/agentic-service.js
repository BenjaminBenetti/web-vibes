...
This ensures you can see the current state before making modifications.`;
  ...
  _trimHistory() {
    ...
    if (trimmedCount > 0) {
      trimmingMessage = `🗂️ Conversation history trimmed: Removed ${trimmedCount} oldest messages to stay under token limit. History reduced from ${initialCount} to ${finalCount} messages (${Math.round(initialSize / 1024)}KB → ${Math.round(finalSize / 1024)}KB).`;
    }
